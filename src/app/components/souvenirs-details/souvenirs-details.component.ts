...
@Component({
  selector: 'app-souvenirs-details',
  templateUrl: './souvenirs-details.component.html',
  styleUrls: ['./souvenirs-details.component.css']
})
export class SouvenirsDetailsComponent implements OnInit {
  ...
  public async setPlaceDetails(name) {
    const souvenirs = JSON.parse(await this.storageService.get("souvenirs"));
    for (const i in souvenirs) {
      if (souvenirs[i].name === name) {
        this.place_details = souvenirs[i];
      }
    }
    ...
    const souvenir_stores = JSON.parse(await this.storageService.get("souvenir_stores"));
    for (const i in souvenir_stores) {
      if (souvenir_stores[i].name === this.place_details.place_id) {
        this.place_details.formatted_address = souvenir_stores[i].formatted_address;
        this.place_details.geometry = souvenir_stores[i].geometry;
        this.place_details.rating = souvenir_stores[i].rating;
        this.place_details.opening_hours = souvenir_stores[i].opening_hours;
        this.place_details.place_review = souvenir_stores[i].review;
      }
    }
  }
