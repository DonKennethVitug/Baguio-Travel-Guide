...
@Component({
  selector: 'app-souvenir-stores-details',
  templateUrl: './souvenir-stores-details.component.html',
  styleUrls: ['./souvenir-stores-details.component.css']
})
export class SouvenirStoresDetailsComponent implements OnInit {
  ...
  public async setPlaceDetails(place_id) {
    const souvenirs = JSON.parse(await this.storageService.get("souvenir_stores"));
    for (const i in souvenirs) {
      if (souvenirs[i].place_id === place_id) {
